...
public class TransactionLegacy implements Closeable {
  ...
  public void registerLock(String sql) {
    if (_txn && s_lockLogger.isDebugEnabled()) {
      final Pair<String, Long> time = new Pair<String, Long>(sql, System.currentTimeMillis());
      _lockTimes.add(time);
    }
  }
  ...
  public static Connection getStandaloneSimulatorConnection() {
    try {
      final Connection conn = s_simulatorDS.getConnection();
      if (s_connLogger.isTraceEnabled()) {
   ...
   * @see java.sql.Connection
   */
  public PreparedStatement prepareAutoCloseStatement(String sql, int resultSetType, int resultSetConcurrency,
    ...
    try {
      ...
      try {
        ...
        final PoolableConnectionFactory simulatorPoolableConnectionFactory = new PoolableConnectionFactory(
            simulatorConnectionFactory, simulatorConnectionPool, new StackKeyedObjectPoolFactory(), null, false, false);
