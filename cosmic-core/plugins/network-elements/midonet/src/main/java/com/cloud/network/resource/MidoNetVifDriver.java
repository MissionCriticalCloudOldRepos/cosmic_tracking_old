...
public class MidoNetVifDriver extends VifDriverBase {
  ...
  public void configure(Map<String, Object> params) throws ConfigurationException {
    ...
    final String midoLoc = (String)params.get("midonet.apiserver.address");
    if (midoLoc != null) {
  ...
  private Host getMyHost(MidonetApi api) {
    final Script command = new Script("/bin/bash", _timeout);
    command.add("-c");
    command.add("awk -F'=' '{if ($1~/host/) print $2}' /etc/midolman/host_uuid.properties");

    final OutputInterpreter.AllLinesParser parser = new OutputInterpreter.AllLinesParser();
    command.execute(parser);
    final String host_uuid = parser.getLines().split("\\n")[0];
    for (final Host host : api.getHosts()) {
  ...
  private String addTap() {
    ...
    command.execute(parser);
    final String tapName = parser.getLines().split("\\n")[0];
  ...
  public LibvirtVmDef.InterfaceDef plug(NicTO nic, String guestOsType, String nicAdapter) throws InternalErrorException, LibvirtException {
    ...
    if (nic.getBroadcastType() == Networks.BroadcastDomainType.Mido && (nic.getType() == Networks.TrafficType.Guest || nic.getType() == Networks.TrafficType.Public)) {
       ...
       */
      final MultivaluedMap qNet = new MultivaluedMapImpl();
      final String nicAuthority = nic.getBroadcastUri().getAuthority();
      final String tenantId = nicAuthority.split("\\.")[0];
      qNet.add("tenant_id", tenantId);
      final String url = nicAuthority.split("\\.")[1];
      final String netName = url.split(":")[0];

      final MidonetApi api = new MidonetApi(_midoApiLocation);
      api.enableLogging();

      for (final Bridge b : api.getBridges(qNet)) {
        if (b.getName().equals(netName)) {
          for (final BridgePort p : b.getPorts()) {
            final UUID pvif = p.getVifId();
  ...
  public void unplug(LibvirtVmDef.InterfaceDef iface) {
    final String netName = iface.getBrName();
