...
public class ElastistorPrimaryDataStoreLifeCycle implements PrimaryDataStoreLifeCycle {
  ...
  public DataStore initialize(Map<String, Object> dsInfos) {
    ...
    final boolean managed = (Boolean) dsInfos.get("managed");
    ...
    if (!managed) {
      ...
      final String[] mp = accesspath.split("/");
      mountpoint = mp[1];
    ...
    final List<StoragePoolVO> storagePoolVO = _storagePoolDao.listAll();
    for (final StoragePoolVO poolVO : storagePoolVO) {
  ...
  private PrimaryDataStoreParameters createElastistorVolume(PrimaryDataStoreParameters parameters, Tsm tsm, String storagePoolName, Long capacityBytes, Long capacityIops, String protocoltype,
    ...
    try {
      ...
      final FileSystem volume = ElastistorUtil.createElastistorVolume(storagePoolName, tsm.getUuid(), capacityBytes, capacityIops, protocoltype, mountpoint);

      if (protocoltype.contentEquals("iscsi")) {
        final String accesspath = "/" + volume.getIqn() + "/0";
        parameters.setPath(accesspath);
      }
  ...
  private StoragePoolType getStorageType(String url) {
    ...
    while (st.hasMoreElements()) {
      final String accessprotocol = st.nextElement().toString();
  ...
  public boolean attachCluster(DataStore store, ClusterScope scope) {
    ...
    final StoragePoolVO dataStoreVO = _storagePoolDao.findById(store.getId());

    final PrimaryDataStoreInfo primarystore = (PrimaryDataStoreInfo) store;
    // Check if there is host up in this cluster
    final List<HostVO> allHosts = _resourceMgr.listAllUpAndEnabledHosts(Host.Type.Routing, primarystore.getClusterId(), primarystore.getPodId(), primarystore.getDataCenterId());
    if (allHosts.isEmpty()) {
  ...
  private boolean createStoragePool(long hostId, StoragePool pool) {
    ...
    if (pool.getPoolType() != StoragePoolType.NetworkFilesystem && pool.getPoolType() != StoragePoolType.Filesystem && pool.getPoolType() != StoragePoolType.IscsiLUN
        && pool.getPoolType() != StoragePoolType.Iscsi && pool.getPoolType() != StoragePoolType.SharedMountPoint
  ...
  public boolean deleteDataStore(DataStore store) {
    ...
    final StoragePoolVO storagePoolVO = _storagePoolDao.findById(store.getId());
  ...
  private void deleteElastistorVolume(StoragePool pool, boolean managed) {
    ...
    final String poolid = pool.getUuid();
    boolean status;
  ...
  public void updateStoragePool(StoragePool storagePool, Map<String, String> details) {
    final String capacityBytes = details.get(PrimaryDataStoreLifeCycle.CAPACITY_BYTES);
    final String capacityIops = details.get(PrimaryDataStoreLifeCycle.CAPACITY_IOPS);

    final StoragePoolVO storagePoolVO = _storagePoolDao.findById(storagePool.getId());

    try {
      if(capacityBytes != null){
        final long usedBytes = _capacityMgr.getUsedBytes(storagePoolVO);
        ...
        final UpdateTsmStorageCmdResponse updateTsmStorageCmdResponse  = ElastistorUtil.updateElastistorTsmStorage(capacityBytes,storagePool.getUuid());
      ...
      if(capacityIops != null){
        ...
        final long usedIops = _capacityMgr.getUsedIops(storagePoolVO);
        final long capacity = Long.parseLong(capacityIops);
        ...
        final UpdateTsmCmdResponse updateTsmCmdResponse   = ElastistorUtil.updateElastistorTsmIOPS(capacityIops,storagePool.getUuid());
