...
public class SolidFirePrimaryDataStoreLifeCycle implements PrimaryDataStoreLifeCycle {
  ...
  public DataStore initialize(Map<String, Object> dsInfos) {
    ...
    final String uuid = SolidFireUtil.PROVIDER_NAME + "_" + zone.getUuid() + "_" + storageVip;
    ...
    try {
      final String clusterDefaultMinIops = SolidFireUtil.getValue(SolidFireUtil.CLUSTER_DEFAULT_MIN_IOPS, url);
    ...
    try {
      final String clusterDefaultMaxIops = SolidFireUtil.getValue(SolidFireUtil.CLUSTER_DEFAULT_MAX_IOPS, url);
    ...
    try {
      final String clusterDefaultBurstIopsPercentOfMaxIops = SolidFireUtil.getValue(SolidFireUtil.CLUSTER_DEFAULT_BURST_IOPS_PERCENT_OF_MAX_IOPS, url);
  ...
  public boolean attachZone(DataStore dataStore, ZoneScope scope, HypervisorType hypervisorType) {
    ...
    final List<HostVO> xenServerHosts = _resourceMgr.listAllUpAndEnabledHostsInOneZoneByHypervisor(HypervisorType.XenServer, scope.getScopeId());
    final List<HostVO> kvmHosts = _resourceMgr.listAllUpAndEnabledHostsInOneZoneByHypervisor(HypervisorType.KVM, scope.getScopeId());
  ...
  public boolean deleteDataStore(DataStore store) {
    final List<SnapshotVO> lstSnapshots = _snapshotDao.listAll();

    if (lstSnapshots != null) {
      for (final SnapshotVO snapshot : lstSnapshots) {
        final SnapshotDetailsVO snapshotDetails = _snapshotDetailsDao.findDetail(snapshot.getId(), SolidFireUtil.STORAGE_POOL_ID);
  ...
  public void updateStoragePool(StoragePool storagePool, Map<String, String> details) {
    final StoragePoolVO storagePoolVo = storagePoolDao.findById(storagePool.getId());
