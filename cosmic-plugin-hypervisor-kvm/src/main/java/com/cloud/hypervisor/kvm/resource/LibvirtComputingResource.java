...
public class LibvirtComputingResource extends ServerResourceBase implements ServerResource, VirtualRouterDeployer {
  ...
  public boolean configure(final String name, final Map<String, Object> params) throws ConfigurationException {
    ...
    if (_guestCpuMode != null) {
      ...
      if (_hypervisorLibvirtVersion < 9 * 1000 + 10) {
        s_logger.warn("Libvirt version 0.9.10 required for guest cpu mode, but version "
            + prettyVersion(_hypervisorLibvirtVersion) +
    ...
    s_logger.debug("Found pif: " + _pifs.get("private") + " on " + _privBridgeName + ", pif: " + _pifs.get("public")
        + " on " + _publicBridgeName);
    ...
    if (_migrateSpeed == -1) {
      ...
      final String speed = Script.runSimpleBashScript(
          "ethtool " + _pifs.get("public") + " |grep Speed | cut -d \\  -f 2");
      if (speed != null) {
  ...
  private void getPifs() {
    ...
    if (_pifs.get("private") == null) {
      s_logger.debug("guest(private) traffic label '" + _guestBridgeName
    ...
    if (_pifs.get("public") == null) {
      s_logger.debug(
          "public traffic label '" + _publicBridgeName + "' not found as bridge, looking for physical interface");
  ...
  private String matchPifFileInDirectory(final String bridgeName) {
    ...
    if (!brif.isDirectory()) {
      ...
      s_logger.debug("failing to get physical interface from bridge " + bridgeName + ", does " + brif.getAbsolutePath()
          + "exist?");
      return "";
    }
    ...
    s_logger.debug("failing to get physical interface from bridge " + bridgeName
  ...
  public LibvirtVMDef createVMFromSpec(final VirtualMachineTO vmTO) {
    ...
    if (_hypervisorLibvirtVersion >= 9000) {
      ...
      /**
       * A 4.0.X/4.1.X management server doesn't send the correct JSON command for getMinSpeed, it only sends a 'speed'
       * field.
       * 
  ...
  public void createVbd(final Connect conn, final VirtualMachineTO vmSpec, final String vmName, final LibvirtVMDef vm)
    ...
    for (final DiskTO volume : disks) {
      ...
      if (_diskActivityCheckEnabled && physicalDisk != null && physicalDisk.getFormat() == PhysicalDiskFormat.QCOW2) {
        s_logger.debug("Checking physical disk file at path " + volPath
            + " for disk activity to ensure vm is not running elsewhere");
        try {
          HypervisorUtils.checkVolumeFileForActivity(volPath, _diskActivityCheckTimeoutSeconds,
              _diskActivityInactiveThresholdMilliseconds, _diskActivityCheckFileSizeMin);
        } catch (final IOException ex) {
    ...
    if (HypervisorType.LXC.toString().toLowerCase().equals(vm.getHvsType())) {
      for (final DiskTO volume : disks) {
        ...
        if (volume.getType() == Volume.Type.ROOT) {
          final KVMPhysicalDisk physicalDisk = _storagePoolMgr.getPhysicalDisk(store.getPoolType(), store.getUuid(),
              data.getPath());
  ...
  private void createVif(final LibvirtVMDef vm, final NicTO nic, final String nicAdapter)
    ...
    if (nic.getType().equals(TrafficType.Guest) && nic.getBroadcastType().equals(BroadcastDomainType.Vsp)) {
      ...
      if (s_logger.isDebugEnabled()) {
        s_logger.debug("NIC with MAC " + nic.getMac() + " and BroadcastDomainType " + nic.getBroadcastType()
            + " in network(" + nic.getGateway() + "/" + nic.getNetmask()
  ...
  public StartupCommand[] initialize() {
    ...
    final StartupRoutingCommand cmd = new StartupRoutingCommand((Integer) info.get(0), (Long) info.get(1),
        (Long) info.get(2), (Long) info.get(4), (String) info.get(3), _hypervisorType,
    ...
    try {
      ...
      final KVMStoragePool localStoragePool = _storagePoolMgr.createStoragePool(_localStorageUUID, "localhost", -1,
  ...
  protected List<Object> getHostInfo() {
    ...
    info.add(cpuSockets);
    s_logger.debug("cpus=" + cpus + ", speed=" + speed + ", ram=" + ram + ", _dom0MinMem=" + _dom0MinMem
  ...
  boolean isGuestPVEnabled(final String guestOSName) {
    ...
    if (guestOSName.startsWith("Ubuntu") || guestOSName.startsWith("Fedora 13") || guestOSName.startsWith("Fedora 12")
        ...
        ||
        guestOSName.startsWith("Red Hat Enterprise Linux 5.5") || guestOSName.startsWith("Red Hat Enterprise Linux 6")
